...
app.get("/health", (_req, res) => {
  res.json({ status: "Audit route is Fine!" });
});

/** ROUTES */
app.use("/api/v1/audit", urlRoute);
/**
...
app.get("/metrics", async (req, res) => {
  try {
    ...
    logger.info("Audit Metrics has been scraped successfully!");
  } catch (error) {
    logger.error("Audit Metrics scraping error:", { error });
